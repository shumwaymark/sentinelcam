...
This is a list of object class names that are of interest for detection and tracking. Only objects
matching one of these names will trigger event capture and tracking. The class names must match those
provided by the object detection model in use.

.. code-block:: yaml

  interesting_objects: ['person', 'vehicle', 'dog', 'car', 'truck']
