    ...
    def _render_montage(viewname, feed, events, output_path, include_overlays,
        ...
        try:
            ...
            for frame_idx, (date, event, frametime, tracking, imgsize) in enumerate(all_frames_data):
                ...
                try:
                    # A frame whose file is gone comes back as the datapump's 1x1
                    # placeholder, not an error — skip it rather than write a black frame
                    # into the montage (scene frames expire; see crop_overlay).
